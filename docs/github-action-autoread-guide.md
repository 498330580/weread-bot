...
1. 访问 [WeRead Bot 项目主页](https://github.com/498330580/weread-bot)
2. 点击右上角的 **Fork** 按钮
3. 选择你的 GitHub 账户，完�?Fork
...
| Secret 名称 | 说明 | 获取方法 |
|------------|------|----------|
| `WEREAD_CURL_STRING` | 微信读书�?cURL 请求字符�?| 参考主文档�?获取 cURL 字符�?章节 |
...
> **获取方式**：支�?Discord、Slack、Email 等数十种服务，详�?[Apprise 文档](https://github.com/caronc/apprise)

**飞书通知**
| Secret 名称 | 说明 |
|------------|------|
| `FEISHU_WEBHOOK_URL` | 飞书机器�?Webhook URL |
...
| Secret 名称 | 说明 |
|------------|------|
| `WEWORK_WEBHOOK_URL` | 企业微信机器�?Webhook URL |
...
| Secret 名称 | 说明 |
|------------|------|
| `DINGTALK_WEBHOOK_URL` | 钉钉机器�?Webhook URL |
...
| Secret 名称 | 说明 | 默认�?|
|------------|------|--------|
| `HACK_COOKIE_REFRESH_QL` | Cookie刷新时ql属性值设�?| `false` |

> **Hack 配置说明�?*
> - `HACK_COOKIE_REFRESH_QL`: 控制Cookie刷新请求中的`ql`参数�?
>   - `false` (默认): 使用`"ql": false`
>   - `true`: 使用`"ql": true`
> - 根据不同用户的环境，可能需要设置为True或False来确保cookie刷新正常工作
> - 如果遇到cookie刷新失败的问题，可以尝试切换此配置的�?
> - 建议先使用默认值，如果出现cookie相关错误再尝试修�?

### 步骤 3: 启用 GitHub Actions

1. 在你的仓库中点击 **Actions** 选项�?
2. 如果看到"Actions are disabled"提示，点�?**I understand my workflows, go ahead and enable them**
...
1. �?**Actions** 页面，选择 **Auto Reading Bot** workflow
2. 点击 **Run workflow** 按钮
3. 配置运行参数�?
   - **阅读时长**: 格式�?`60-90`，表示随�?60-90 分钟
   - **阅读模式**: 选择阅读策略
   - **启用通知**: 是否发送完成通知

#### 定时触发（可选）

如需启用定时运行，修�?`.github/workflows/auto-reading.yml` 文件�?
...
如需更详细的配置，可以修�?workflow 文件中的 config.yaml 内容�?
...
A: 
1. 重新从微信读书网页版获取 cURL 字符�?
2. 在仓�?Settings �?Secrets 中更�?`WEREAD_CURL_STRING`
...
A: 是的，GitHub 免费账户每月�?2000 分钟�?Actions 使用时间。本项目单次运行通常消�?60-120 分钟�?

### Q: 如何设置定时运行�?

A: 取消 workflow 文件�?schedule 部分的注释，并根据需要调�?cron 表达式。注意时间为 UTC 时间�?

### Q: 遇到 Cookie 刷新失败怎么办？

A: 这可能是 `cookie_refresh_ql` 配置问题�?
1. 在仓�?Settings �?Secrets 中添�?`HACK_COOKIE_REFRESH_QL`
2. 如果当前设置�?`false`，尝试设置为 `true`
3. 如果当前设置�?`true`，尝试设置为 `false`
4. 重新运行 Action 测试
...
- 如果看到 Cookie 相关错误，尝试调�?`HACK_COOKIE_REFRESH_QL` 配置
- 如果看到 401/403 认证错误，也可能是此配置问题

## 🔒 安全提示

1. **不要在公开仓库中暴�?* cURL 字符串或其他敏感信息
2. **定期更新** cURL 字符串以保持有效�?
3. **合理设置**运行频率，避免被检测为异常行为
4. **及时关注**账户状态，如有异常立即停止使用

---

如有问题或建议，欢迎�?[Issues](../../issues) 中反馈�?
